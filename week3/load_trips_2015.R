...
trips_per_day_2015 <- trips %>% group_by(ymd) %>% summarize(num_trips=n())
trips_per_day_2015 <- inner_join(trips_per_day_2015,weather,by="ymd") %>% filter(ymd!="2016-01-01") %>% select(-date) %>% mutate(tmax=tmax/10,tmin=tmin/10)
